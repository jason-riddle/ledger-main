"""SPS Mortgage Servicing PDF text parser."""
...
def render_pdf_text(text: str, config: Optional[SPSConfig] = None) -> str:
    """Render SPS statement text into Beancount entries."""
    entries = parse_pdf_text(text, config=config)
    return rendering.render_to_beancount(entries)


def render_pdf_text_to_jsonl(text: str, config: Optional[SPSConfig] = None) -> str:
